...
def display_debt_reduction(plan: Dict[str, Any]):
    ...
    if "debts" in plan:
        ...
        if not debt_df.empty and all(col in debt_df.columns for col in ['name', 'amount', 'interest_rate']):
            fig = px.bar(
                debt_df,
                x="name",
                y="amount",
            ...
            st.warning("Debt data is missing or has incorrect column names. Please ensure each debt has a name, amount, and interest rate.")
